...
germline_maxwell_brca2 <-   separate(germline_mut_type_Maxwell_2017, mutation, into = c("brca_status", "mutation"),  sep = " ") %>%  
  ...
  rename(name = tumor_id) # some have two spaces as delimiter so the mutation info has been discarded in these cases but is in the original table
...
pancan_BRCA_LOF <- full_join(pancan_BRCA_LOF, missing_gBRCA2LOH) %>% 
  mutate(contrast_group = "BRCA1/2_loss")
...
```
```{r}
#save a copy of the pancancer clincal data with the contrast labels added
 write_delim(pancan_contrast, "./Data/Processed/20230803_ST_58_Clinical_data_TCGA-OV_contrast_groups.tsv", delim = "\t") # save with contrast groups added
```
