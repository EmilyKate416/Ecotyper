...
rawCounts <- rawCounts[,!colnames(rawCounts) %in% missing_contrast$submitter_id]
...
dge <- DGEList(counts = rawCounts, genes = annotated_genes$gene_id, samples = meta_rawCounts_matched, group = meta_rawCounts_matched$contrast_group) # contains all samples in one object 
...
modelMatrix <- model.matrix(~ 0 + group , data=dgeFilExpNorm)
...
# voom transformation
voom <- voom(dgeFilExpNorm, modelMatrix, plot = TRUE) 
...
contr <- makeContrasts( groupHRD_high - groupBRCA1_2_loss, groupHRD_low - groupBRCA1_2_loss, levels = modelMatrix)

contr
contrastsfit <- contrasts.fit(fit, contrasts = contr)
fittedContrasts <- eBayes(contrastsfit, robust = TRUE) # use robust emperical bayes for statitsical testing to account for hypervariable genes
...
head(results_HRDhigh, 20) # look at top 20 DE genes (sorted by p value)
write_csv(topTable, "I:/research/jblab/data/group_folders/emily_lythgoe/20230724_EcoTyper_2nd_year/Output/topTableDEgenes_BRCA1_2_loss_vs_HRD_high.csv")

results_HRDlow <- topTable(fittedContrasts, coef= "groupHRD_low - groupBRCA1_2_loss", sort.by = "P", n = Inf)
head(results_HRDlow)
write_csv(topTable, "I:/research/jblab/data/group_folders/emily_lythgoe/20230724_EcoTyper_2nd_year/Output/topTableDEgenes_BRCA1_2_loss_vs_HRD_low.csv")
```
**Statistical testing.**
...
length(which(topTable$adj.P.Val < 0.05 & abs(topTable$logFC) >= 1.0 ))

# Total number of DE genes based on adjusted p value < 0.01 and |logFC| >= 1.0
length(which(topTable$adj.P.Val < 0.01 & abs(topTable$logFC) >= 1.0 ))

dt <- decideTests(tmptest, p.value = 0.05, lfc = 1.0)
summary(dt)
