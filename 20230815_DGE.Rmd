...
ecotype_abundances <- ecotype_abundances %>%
 dplyr::filter(!str_detect( submitter_id, "02A")) %>%  # should I discard secondary?
  dplyr::select(-submitter_id) %>% 
...
CE <- interaction(dgeFilExpNorm$samples$dominant_CE)
CEcol <- CE
levels(CEcol) <- c("darkorange", "cornflowerblue", "darkolivegreen3", "deeppink3", "darkorchid", "brown1", "darkgrey", "chartreuse", "#F8766D", "#00BFC4" , "black", "pink") 
CEcol <- as.character(CEcol) 

plotMDS(dgeFilExpNorm, col = CEcol, labels = NULL, pch = 20)
legend("topright", legend = c("CE1", "CE2", "CE3", "CE4", "CE5", "CE6", "CE7", "CE8", "CE9", "CE10", "Unassigned", "NA"), fill =  c("darkorange", "cornflowerblue", "darkolivegreen3", "deeppink3", "darkorchid", "brown1", "darkgrey", "chartreuse",  "#F8766D", "#00BFC4", "black", "pink" ) )


```
**Looks like I could account for CE within the model? Some separation **
