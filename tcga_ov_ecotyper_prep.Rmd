...
query <- GDCquery(project = "TCGA-OV", 
                  ...
                  workflow.type = "HTSeq - FPKM", #FPKM - A normalized expression value that takes into account each gene length and the number of reads mapped to all protein-coding genes
...
raw.counts$ensembl_gene_id_version <- substr(raw.counts$ensembl_gene_id_version, 1,15) 
    # truncate to remove version number 
raw.counts <- raw.counts %>%  rename(ensembl_gene_id=ensembl_gene_id_version)# rename to reflect that it no longer lists version


raw.counts.named <- left_join(raw.counts, TCGA_genes_matched, by="ensembl_gene_id")

nrow(raw.counts.named[is.na(raw.counts.named$external_gene_name),])

missing_names <- raw.counts.named %>% filter(is.na(external_gene_name))
```
```{r}
raw.counts.named <- raw.counts.named %>% 
