...
gene_clusterA <- gene_cluster1 %>% rownames_to_column("ENSEMBL")

#separate out 1st gene cluster
gene_cluster1_ORA1 <- 
  left_join(gene_clusterA, results_HRDlow) %>% 
#  filter(cluster==1) %>% 
  pull(ENTREZID) %>% 
  as.numeric()
...
#2nd gene cluster 
 ...
 as_tibble(keggReslow11)
 as_tibble(keggReslow12)
 browseKEGG(keggReslow11, "hsa00190")
``` 

```{r, enrichment analysis/ORA, HRD high vs BRCA1/2 loss - comparing gene clusters from heatmap}
#take the pheatmap gene clusters that are DE between HRDlow vs BRCA1/2 loss 
gene_clusterB <- gene_cluster2 %>% rownames_to_column("ENSEMBL")

#separate out 1st gene cluster
gene_cluster2_ORA2 <- 
  left_join(gene_clusterB, results_HRDhigh) %>% 
#  filter(cluster==1) %>% 
  pull(ENTREZID) %>% 
  as.numeric()
...
#1st gene cluster
 keggReslow21 <- enrichKEGG(gene= gene_cluster1_ORA1, organism = 'hsa')
#2nd gene cluster 
  keggReslow22 <- enrichKEGG(gene= gene_cluster1_ORA2, organism = 'hsa')
 
 as_tibble(keggReslow21)
 as_tibble(keggReslow22)
``` 

##If I increase the gene number from top 300 to top 500 in the heatmap then I get results for the ORA for each comparison, top hit = OXPHOS pathway

Studies in breast and ovarian cancer HRD models depict a metabolic shift that includes enhanced expression of the oxidative phosphorylation (OXPHOS) pathway and its key components and a decline in the glycolytic Warburg phenotype. Hence, HRD cells are more sensitive to metformin and NAD+ concentration changes.
