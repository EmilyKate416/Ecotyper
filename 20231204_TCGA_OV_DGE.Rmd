...
# Select the top genes based on adj.P.Val
  ...
  ha1 <- HeatmapAnnotation(df=voom$targets$group, col = list(df = c("BRCA1_2_loss" = "red", "HRD_high" = "green", "HRD_low" = "salmon"))) 
...
gene <- enframe(cuthc, name="GeneID", value="Group") %>% 
  select(Group)
ha2 <- HeatmapAnnotation(df=gene, which="row", col = list(Group = c("1" = "red", "2" = "green"))) 
