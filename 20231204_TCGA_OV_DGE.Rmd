...
library(circlize)#only require if I use alternatie heatmap method, otherwise need "ComplexHeatmap"
...
library(pheatmap)
gene_cluster <- data.frame(cluster = cuthc)
col_annot <- data.frame(group=voom$targets$group,
                    ecotype=voom$targets$carcinoma_ecotype)
colour_list <- list(cluster = c("1" = "cadetblue1", "2" = "darkcyan"),
                    group = c("BRCA1_2_loss" = "pink", 
                                             "HRD_high" = "cornflowerblue", 
                                             "HRD_low" = "purple"),
                     ecotype= c("CE1"="darkorange", "CE2"="yellow", 
                               "CE3"="darkolivegreen1", "CE4"="deeppink3", 
                               "CE5"="coral4", "CE6"="darkslateblue", 
                               "CE7"="red", "CE8"="chartreuse4", 
                               "CE9"="darkgoldenrod4", "CE10"="#00BFC4" , 
                               "NA"="grey"))
pheatmap(z.mat, 
...
library(pheatmap)
gene_cluster <- data.frame(cluster = cuthc)
col_annot <- data.frame(contrast_group=voom$targets$group,
                    ecotype=voom$targets$carcinoma_ecotype,
                    submitter_id=voom$targets$submitter_id) %>% 
  filter(contrast_group != "HRD_high") %>% 
  mutate(submitter_id = str_replace_all( submitter_id,"-", ".")) 

plotDatlow <- data.frame(plotDat) %>% 
    select(any_of(col_annot$submitter_id)) %>% 
    as.matrix()
#z-scale the data (results = number of SD the GE is from the mean for that gene 
#across all samples)
  z.mat <- t(scale(t(plotDatlow), center=TRUE, scale=TRUE))
